...
public final class FirstPartTasks {
    ...
    public static List<String> allTracksSorted(Stream<Album> albums) {
        return albums.map(Album::getTracks).flatMap(Collection::stream).map(Track::getName)
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
                .entrySet().stream()
                .filter(albumLongEntry -> albumLongEntry.getValue() > 1).count();
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(Comparator.comparing(album -> album.getTracks().stream()
                ...
                .average().orElse(0), Comparator.<Double>reverseOrder())).collect(Collectors.toList());
    }
